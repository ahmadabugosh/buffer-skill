...
export class BufferApi {
  ...
  async createPost(input) {
    // Map our input to Buffer's parameters
    const variables = {
      text: input.text,
      channelId: input.channelId,
      schedulingType: 'automatic',
      mode: input.now ? 'shareNow' : (input.queue ? 'addToQueue' : 'customSchedule'),
      dueAt: input.scheduledAt || null
    };
    
    const data = await this.gql(CREATE_POST_MUTATION, variables);
